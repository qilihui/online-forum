...
import com.qilihui.forum.util.BaiduTextUtil;
...
public class QuestionServiceImpl implements QuestionService {
    ...
    private final BaiduTextUtil baiduTextUtil;

    private final QuestionDingDao questionDingDao;

    @Autowired
    public QuestionServiceImpl(QuestionMapper questionMapper, QuestionTagMapper questionTagMapper, TagMapper tagMapper, UserRateMapper userRateMapper, UserInfoMapper userInfoMapper, CommentMapper commentMapper, FollowMapper followMapper, BaiduTextUtil baiduTextUtil, QuestionDingDao questionDingDao) {
        ...
        this.baiduTextUtil = baiduTextUtil;
        this.questionDingDao = questionDingDao;
    }
    ...
    public List<Question> queryAllQuestionsWithCurrPage(Integer currPage) throws ParseException {
        ...
        for (int i = 0; i < questionList.size(); i++) {
            ...
            if (question.getIsDing() != 1) {
                ...
                QuestionDing dingQ = questionDingDao.selectOneByQid(id);
                if (dingQ == null) {
                    continue;
                }
                if (sdf.format(date).compareTo(dingQ.getEndTime()) >= 0) {
                    dingQ.setStatus(1);
                    questionDingDao.updateByPrimaryKey(dingQ);
                    question.setIsDing(0);
                    questionMapper.updateDingStatusByQuestionId(0, question.getId());
                    i--;
                }
            }
        }
        ...
        reOrderList.addAll(dingQuestions);
    ...
    @Override
    public List<Question> getAllQuestionWithFollow(int uid) {
        return questionMapper.getAllQuestionWithFollow(uid);
    }
    ...
    @Override
    @Transactional
    public boolean insertQuestion(Question question, Integer ding) throws RateException {
        String htmlContent = HtmlUtil.cleanHtmlTag(question.getDescription());
        boolean normal = baiduTextUtil.textIsNormal(question.getTitle() + " " + question.getTag() + " " + htmlContent);
        if (!normal) {
            return false;
        }
        if (ding == 1 || ding == 2) {
            question.setIsDing(1);
        }
        //插入问题，返回主键
        ...
        if (userRate == null) {
            ...
            if (ding > 0) {
                if (ding == 1) {
                    ...
                    userRateMapper.updateRateById(userInfo.getUId(), rate - 10);
                    questionDingDao.insert(getQuestionDing(questionId, ding));
                } else if (ding == 2) {
                    if (rate < 20) {
                        throw new RateException("积分不足");
                    }
                    userRateMapper.updateRateById(userInfo.getUId(), rate - 20);
                    questionDingDao.insert(getQuestionDing(questionId, ding));
                } else {
                    throw new RateException("信息错误");
                }
            } else {
                //发一贴加3积分
    ...
    public int updateQuestionInfoByIdSelective(Integer questionId, String description, String tag) {
        String htmlContent = HtmlUtil.cleanHtmlTag(description);
        boolean normal = baiduTextUtil.textIsNormal(htmlContent);
        if (!normal) {
            return -1;
        }
        //查询该问题ID对应的原始标签信息
     ...
     */
    @Override
    public int deleteQuestion(Integer questionId, Integer userId) {
    ...
    private QuestionDing getQuestionDing(int qid, int dingDay) {
        QuestionDing qDing = new QuestionDing();
        qDing.setStatus(0);
        qDing.setQId(qid);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        Date date = new Date();
        ...
        calendar.add(Calendar.DATE, dingDay);
        date = calendar.getTime();
        qDing.setEndTime(sdf.format(date));
        return qDing;
    }
}
