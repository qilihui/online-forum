...
import com.qilihui.forum.admin.service.AdminQuestionInfoService;
...
public class AdminQuestionInfoController {
    ...
    @PostMapping("adminDeleteQuestion")
    @ResponseBody
    public ForumResult disableQuestion(@RequestParam("questionId") Integer questionId) {
        if (adminQuestionInfoService.disableQuestion(questionId) == 1)
            return ForumResult.ok();
        else
            return ForumResult.build(400, "删除失败");
    }

    @GetMapping("/adminGetQueryById")
    @ResponseBody
    public ForumResult getQuestionById(@RequestParam("questionId") Integer questionId) {
        Question question = adminQuestionInfoService.getQuestionById(questionId);
        return ForumResult.ok(question);
    }
}
