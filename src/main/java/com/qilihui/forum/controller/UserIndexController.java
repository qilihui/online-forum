...
import com.qilihui.forum.pojo.vo.FollowVO;
import com.qilihui.forum.pojo.vo.UserIndexCommentsVO;
import com.qilihui.forum.pojo.vo.UserIndexInfoVO;
...
public class UserIndexController {
    ...
    public String userIndex(@PathVariable(value = "token") String token, Model model) {
        ...
        try {
            ...
            if (userInfo.getDisable()) {
                model.addAttribute("info", userInfo.getUsername() + "用户被禁用");
                return "error";
            }
            //保存用户信息
    ...
    private FollowVO convertFollowToVO(Follow follow) {
        ...
        //头像
        UserInfo info = userInfoService.selectUserInfoByUid(follow.getUserId());
        return convertToVo(vo, info);
    }
    ...
    public FollowVO convertToVo(FollowVO vo, UserInfo info) {
