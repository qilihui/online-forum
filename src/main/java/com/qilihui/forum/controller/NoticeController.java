...
import com.qilihui.forum.pojo.vo.*;
...
public class NoticeController {
    ...
    @Autowired
    public NoticeController(CollectionService collectionService, CommentNoticeService commentNoticeService, UserInfoService userInfoService, QuestionService questionService, FollowService followService, CommentLikeService commentLikeService, CommentService commentService, QuestionReportService questionReportService) {
    ...
    @ResponseBody
    //TODO 分页使用layuitable结果集
    public ForumResult getReport(@RequestParam("page") Integer page,
                                 @RequestParam("username") String username) {
        PageHelper.startPage(page, 5);
        List<ReportNoticeVo> voList = questionReportService.showRes(username);
        if (voList.size() == 0) {
            return ForumResult.ok();
        }
        return ForumResult.ok(voList);
    }
}
